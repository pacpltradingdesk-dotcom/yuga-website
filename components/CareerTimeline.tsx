      ...
      {CAREER_TRACK.map((entry, index) => (
        ...
        >
          ...
          <div className="relative flex flex-col items-center">
            <div className="w-5 h-5 rounded-full bg-background border-2 border-accent mt-1 shrink-0 flex items-center justify-center group-hover:scale-125 group-hover:border-accent-light transition-all duration-300 shadow-[0_0_12px_rgba(245,158,11,0.4)] group-hover:shadow-[0_0_20px_rgba(245,158,11,0.7)]">
              <div className="w-1.5 h-1.5 rounded-full bg-accent group-hover:bg-accent-light transition-all duration-300" />
            </div>
          </div>
          
          {/* Content Card */}
          <div className="flex-1 glass p-6 rounded-2xl border border-border group-hover:border-accent/20 group-hover:glass-hover transition-all duration-300 -mt-3 shadow-[0_4px_20px_rgba(15,23,42,0.05)] group-hover:shadow-[0_4px_30px_rgba(245,158,11,0.05)]">
            <p className="font-display font-extrabold text-primary text-xl mb-2 group-hover:text-accent transition-colors">{entry.company}</p>
            <p className="text-secondary text-sm mb-3 flex items-center gap-2">
              ...
              {entry.location} · <span className="text-primary/80 font-medium">{entry.plantType}</span>
            </p>
